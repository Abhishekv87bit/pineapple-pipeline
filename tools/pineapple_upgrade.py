            ...
            "templates": [
                {
                    ...
                    "project_version": t.project_version,
                    "diff_lines": t.diff_lines,
                }
        ...
        if project_version == current_version:
            ...
            report.templates.append(
                TemplateStatus(
                    ...
                    current_version=current_version,
                    project_version=project_version,
                    diff_lines=len([ln for ln in diff if ln.startswith("+") or ln.startswith("-")]),
                )
            )
    ...
    if args.json:
        ...
        for t in report.templates:
            ...
            if t.status == "outdated":
                ...
                print("         Not in project (run apply_pipeline.py to add)")
            elif t.status == "custom":
                print("         Custom file (no version header, skipped)")

        if report.outdated_count > 0:
            print(f"\n{report.outdated_count} template(s) need updating.")
            print("Use --diff <template> to see changes.")
        else:
